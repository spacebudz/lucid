import { createContext, ReactNode, useContext, useState } from "react"
import {
    Lucid,
    Blockfrost,
    Lovelace,
    Data,
    SpendingValidator,
    TxHash
} from "lucid-cardano"
import assert from "assert"


type lucidContextType = {
    // lucid?: Lucid
    // wallet?: WalletAPI
    connect: any
    lockUtxoAlways: any
    redeemUtxoAlways: any
    lockUtxo: any
    redeemUtxo: any
}

const lucidContextDefaultValues: lucidContextType = {
    connect: undefined,
    lockUtxoAlways: undefined,
    redeemUtxoAlways: undefined,
    lockUtxo: undefined,
    redeemUtxo: undefined
}

const LucidContext = createContext<lucidContextType>(lucidContextDefaultValues)

export function useLucid() {
    return useContext(LucidContext)
}

type Props = {
    children: ReactNode
}

export function LucidProvider({ children }: Props) {

    // const lucid: Promise<Lucid> = Lucid.new(
    //     new Blockfrost('https://cardano-testnet.blockfrost.io/api/v0', '<projectId>'),
    //     'Testnet'
    // )

    // const nami: Promise<WalletAPI> = window.cardano.nami.enable()

    const [lucid, setLucid] = useState<Lucid>()
    const [wallet, setWallet] = useState<WalletAPI>()
    
    const connectLucid = async () => {
        if (!lucid) {
            console.log("lucid not found, connecting")
            const l: Lucid = await Lucid.new(
                new Blockfrost('https://cardano-testnet.blockfrost.io/api/v0', 'testnetrH93Gogbn6ld9GMVQDAjSCh5KKmU3Vlq'),
                'Testnet'
                )
            setLucid(l)
            return true
        } else {
            return false
        }
    }

    const connectNami = async () => {
        if (window.cardano && window.cardano.nami) { 
            return await window.cardano.nami.enable()
        } else {
            alert("please enable or install Nami wallet")
        }
    }
        
    const connectWallet = async () => {
        if (!wallet) {
            console.log("wallet not found, connecting")
            const w = await connectNami()
            setWallet(w)
            return true
        } else {
            return false
        }
    }

    const connectLucidAndWallet = async () => {
        console.log("connecting lucid and wallet")
        const [lucidFreshConnect, walletFreshConnect] = await Promise.all([
            connectLucid(),
            connectWallet()
        ])
        // const lucidFreshConnect = await connectLucid()
        // const walletFreshConnect = await connectWallet()
        console.log("ensured lucid and wallet are connected")

        // // TODO do this only once (tried semaphore unsucessfully)
        if (lucidFreshConnect) console.log("lucid (freshly) connected")
        if (walletFreshConnect) console.log("wallet (freshly) connected")
        // if (lucidFreshConnect || walletFreshConnect) {
        //     // Assumes you are in a browser environment
        //     console.log("connecting wallet to lucid")
        //     console.log(lucid)
        //     console.log(wallet)
        //     if (lucid) lucid.selectWallet(wallet)
        //     else throw "lucid not found at a point where it should be found"
        // }
    }

    const ensureWalletSelected = () => {
        if (!lucid || !wallet) { 
            alert("connect first")
            return false
        } else {
             // await selectWalletFrom(wallet, lucid)
            lucid.selectWallet(wallet)
            return true
         }
    }

    // attempt to use alternative to selectWallet (broken) but here blockfrost breaks
    const selectWalletFrom = async (wallet: WalletAPI, lucid: Lucid) => {
        const address = (await wallet.getUsedAddresses())[0]
        console.log(address)

        lucid.selectWalletFrom({address}) //, utxos, collateral, rewardAddress)
        console.log(await lucid.wallet.address())
        console.log(await lucid.wallet)
    }

    const NumDatum = (number: number) => Data.to(BigInt(number))
    const NumRedeemer = (number: number) => Data.to(BigInt(number))
    const NullDatum = () => Data.empty()
    const NullRedeemer = () => Data.empty()
    
    const v1matchingNumberScript: SpendingValidator = {
        type: 'PlutusV1',
        script:
          '',
      }

    const v2alwaysSucceedScript: SpendingValidator = {
        type: "PlutusV2",
        script: "49480100002221200101",
    }
    
    const v1matchingNumberAddress = (lucid: Lucid) => lucid.utils.validatorToAddress(
        v1matchingNumberScript
    )

    const v2alwaysSucceedAddress = (lucid: Lucid) => lucid.utils.validatorToAddress(
        v2alwaysSucceedScript,
    )
    
    const lockUtxoAlways = async (
        lovelace: Lovelace,
      ): Promise<TxHash | undefined> => {
        if (!ensureWalletSelected()) return
        assert(lucid)
        const tx = await lucid
          .newTx()
          .payToContract(v2alwaysSucceedAddress(lucid), { inline: NullDatum() }, { lovelace })
          .payToContract(v2alwaysSucceedAddress(lucid), {
            asHash: NullDatum(),
            scriptRef: { type: "PlutusV2", script: "49480100002221200101" }, // adding plutusV2 script to output
          }, {})
          .complete();
      
        const signedTx = await tx.sign().complete();
      
        const txHash = await signedTx.submit();
      
        return txHash;
      };

    const lockUtxoMatching = async (
        number: number,
        lovelace: Lovelace
        ) => { //: Promise<TxHash> => {
        if (!ensureWalletSelected()) return
        assert(lucid)

        console.log("Locking with " + number)

        // console.log(matchingNumberAddress(lucid))
        // addr_test1wz8jmzsx9uh2pgcxj7za36jeln7sprheumhkd3srnytfacg6cgclw

        const tx = await lucid
            .newTx()
            .payToContract(v1matchingNumberAddress(lucid), NumDatum(number), { lovelace })
            .complete()

        const signedTx = await tx.sign().complete()

        const txHash = await signedTx.submit()

        return txHash
    }

    const redeemUtxoAlways = async (): Promise<TxHash | undefined> => {
        if (!ensureWalletSelected()) return
        assert(lucid)
        const referenceScriptUtxo = (await lucid.utxosAt(v2alwaysSucceedAddress(lucid))).find(
            (utxo) => Boolean(utxo.scriptRef),
          );
          if (!referenceScriptUtxo) throw new Error("Reference script not found");
        
          const utxo = (await lucid.utxosAt(v2alwaysSucceedAddress(lucid))).find((utxo) =>
            utxo.datum === NullDatum() && !utxo.scriptRef
          );
          if (!utxo) throw new Error("Spending script utxo not found");
        
          const tx = await lucid
            .newTx()
            .readFrom([referenceScriptUtxo]) // spending utxo by reading plutusV2 from reference utxo
            .collectFrom([utxo], NullRedeemer())
            .complete();
        
          const signedTx = await tx.sign().complete();
        
          const txHash = await signedTx.submit();
        
          return txHash;
      };
    
    const redeemUtxoMatching = async (number: number): Promise<TxHash | undefined> => {
        if (!ensureWalletSelected()) return
        assert(lucid)

        const utxo = (await lucid.utxosAt(v1matchingNumberAddress(lucid))).slice(-1)[0]
      
        const tx = await lucid
          .newTx()
          .collectFrom([utxo], NumRedeemer(number))
          .attachSpendingValidator(v1matchingNumberScript)
          .complete()
      
        const signedTx = await tx.sign().complete()
      
        const txHash = await signedTx.submit()
      
        return txHash
    }
    
    const value = { 
        connect: connectLucidAndWallet,
        lockUtxoAlways: lockUtxoAlways,
        redeemUtxoAlways: redeemUtxoAlways,
        lockUtxo: lockUtxoMatching, 
        redeemUtxo: redeemUtxoMatching
    }
    return (
        <>
            <LucidContext.Provider value={value}>
                {children}
            </LucidContext.Provider>
        </>
    )

}


//////////////

