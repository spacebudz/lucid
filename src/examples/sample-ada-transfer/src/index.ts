...
const handleAdaTransfer = async () => {
  const addr = (document.getElementById("addr-input") as HTMLInputElement)?.value;
  const amt = (document.getElementById('snd-amt') as HTMLInputElement)?.value;
  const network = (document.getElementById('network') as HTMLInputElement)?.value;
  const walletProvider = (document.getElementById('wallet-provider') as HTMLInputElement)?.value;
