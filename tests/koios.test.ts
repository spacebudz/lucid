import {KoiosProvider} from "../src/provider/koios.ts";
...
Deno.test("getDelegation", async () => {
    const koios = new KoiosProvider("Mainnet")
    try {
        const delegation: Delegation = await koios.getDelegation('stake1uyrx65wjqjgeeksd8hptmcgl5jfyrqkfq0xe8xlp367kphsckq250');
        assert(delegation);
    } catch (e) {
        console.log(e)
    }
